...
noPassword can be easily extend. Besides template adaptations it is possible
to send send different messages like SMS. To make those changes you will need
to write a custom login form.
...
custom user model with a unique ``phone_number`` field::
    ...
    from nopassword.forms import BaseLoginForm
...
.. autoclass:: nopassword.forms.BaseLoginForm
    :members:
